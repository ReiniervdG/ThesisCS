...
def structuredIntros (tacSeq : TSyntax ``tacticSeq) (oldGoal : MVarId) : TacticM Unit := do
  ...
  | [newGoal] => 
    ...
    let declsNotInNew := diffLCtx oldLCtx newLCtx
    let declsNotInOld := diffLCtx newLCtx oldLCtx    
    if !isSameGoal ∧ declsNotInNew.size == 0 ∧ declsNotInOld.size > 0 then
      -- Gather idents for $ids
      let mut ids : Array (TSyntax [`ident, `Lean.Parser.Term.hole]) := #[]
      for decl in declsNotInOld do
        if decl.userName.hasMacroScopes then
          ids := ids.push (mkNode `Lean.Parser.Term.hole #[mkAtomFrom (← getRef) "_"])
        else 
          ids := ids.push (mkIdent decl.userName)
        addTrace `xx m!"{decl.userName} : {decl.type}, isAutoGeneratedName: {decl.userName.hasMacroScopes})"
      let test ← `(tactic|intros $[$ids]*)
      addTrace `xx m!"TestStx: {test}"

      let tac ← `(tactic|fix (ha: α) ⊢ α)
      addTrace `structured m!"Try this: {tac}"
    else
...
example : α → β → α := by
  structured intros ha _  -- fix (_ : α) (_ : β)
  sorry