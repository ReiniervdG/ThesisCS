...
-- -- TODO: Change myEvalAlts to return list of MVars
-- -- `getAltMVars` or something
-- def myEvalAlts (elimInfo : ElimInfo) (alts : Array (Name × MVarId)) (optPreTac : Syntax) (altsSyntax : Array Syntax)
--     (initialInfo : Info)
...
--     -- TODO foreach, add mvar to list
--     for (altName, altMVarId) in alts do
--       trace[Elab.induction] "Alternative {altName}"
...
--       | some altStx =>
--         trace[Elab.induction] "After creating altStx, I match on some altStx {altStx}"
...
--           -- NOTE TODO: This is actually the new goal post match statement
--           trace[Elab.induction] "Test01: {(← instantiateMVars (← altMVarId.getDecl).type)}"
--           altMVarId.withContext do
--             for ldecl in (← getLCtx) do
--               trace[Elab.induction] "Test02, {ldecl.userName}: {(← instantiateMVars ldecl.type)}"        
...
--                 altMVarId'.withContext do
--                   trace[Elab.induction] m!"Test03, {(← instantiateMVars (← altMVarId'.getDecl).type)}"
--                   altMVarId'.withContext do
--                     for ldecl in (← getLCtx) do
--                       trace[Elab.induction] "Test04, {ldecl.userName}: {(← instantiateMVars ldecl.type)}"  
...
--       trace[Elab.induction] m!"TEST: {(← instantiateMVars (← goal.getDecl).type)}"
...
-- def myEvalInduction : Tactic := fun stx =>
...
--         -- let x ← result.alts.mapM (fun (name, mvar) => do
--         --   let y ← mvar.getDecl
--         --   let z ← instantiateMVars y.type
--         --   trace[Elab.induction] "{name}: {z}"
--         --   for ldecl in y.lctx do
--         --     trace[Elab.induction] "alts, {ldecl.userName}: {(← instantiateMVars ldecl.type)}"
--         --   )
--         -- let x ← result.others.mapM (fun mvar => do
--         --   let y ← mvar.getDecl
--         --   let z ← instantiateMVars y.type
--         --   trace[Elab.induction] "_: {z}"
--         --   for ldecl in y.lctx do
--         --     trace[Elab.induction] "others, {ldecl.userName}: {(← instantiateMVars ldecl.type)}"
--         --   )
...
--         -- mvarId.withContext do
--         --   for ldecl in (← getLCtx) do
--         --     trace[Elab.induction] "test, {ldecl.userName}: {(← instantiateMVars ldecl.type)}"

--         ElimApp.myEvalAlts elimInfo result.alts optPreTac alts initInfo (numGeneralized := n) (toClear := targetFVarIds)
...
-- def myEvalCases : Tactic := fun stx =>
...
--         mvarId.assign result.elimApp
--         ElimApp.myEvalAlts elimInfo result.alts optPreTac alts initInfo (numEqs := targets.size) (toClear := targetsNew)
...
-- elab &"myTestTactic " t:term " : " t2:ident  " : " t3:(ident) : tactic => do
--   let case1 := mkIdent "zero"
--   let case2 := mkIdent "succ"
--   let indAlt1 ← `(inductionAlt| | $case1 => sorry)
--   let indAlt2 ← `(inductionAlt| | $case2 $t2 $t3 => sorry)
--   myEvalInduction (← `(tactic|induction $t with $indAlt1:inductionAlt $indAlt2:inductionAlt))

-- elab &"myTestTactic2 " t:casesTarget " : " t2:ident : tactic => do
--   let case1 := mkIdent "zero"
--   let case2 := mkIdent "succ"
--   let indAlt1 ← `(inductionAlt| | $case1 => sorry)
--   let indAlt2 ← `(inductionAlt| | $case2 $t2 => sorry)
--   myEvalCases (← `(tactic|cases $t with $indAlt1:inductionAlt $indAlt2:inductionAlt))
...
--   -- Q : Why do goals contain _uniq, even if the relevant mvar seems named in ldecls?
--   myTestTactic (n) : m : m.ih 
--   -- myTestTactic2 (n) : m
--   -- cases n with
--   -- | zero => sorry
--   -- | succ m => sorry
      ...
      | none =>
        ...
        | some (altMVarId', _) =>
          ...
          if !hasAlts then
            ...
            altMVarIds.forM fun mvarId => admitGoal mvarId
      | some altStx =>
        (subgoals, usedWildcard, mvars) ← withRef altStx do
          let unusedAlt :=
            if isWildcard then
              pure (#[], usedWildcard, [])
            else
...
def getMVarsFromMatch (oldGoal : MVarId) (matchTactic : TSyntax `tactic) (isInduction : Bool) : TacticM (Array MVarId) := do
  withoutModifyingState do 
    match isInduction with
    | true => return (← getMVarsFromInductionMatch matchTactic.raw)
    | false => return (← getMVarsFromCaseMatch matchTactic.raw)
